...
describe("ReconnectingWebSocket", () => {
  ...
  it("should keep the pending reconnect timeout when scheduleReconnect is called multiple times", () => {
    const ws = new ReconnectingWebSocket("ws://test", {
      WebSocketConstructor: FakeWebSocket as any,
    ...
    const pendingReconnectTimeout = ws.reconnectTimeout;
    const setTimeoutCallsBeforeDuplicate = setTimeoutCalls;
    const clearTimeoutCallsBeforeDuplicate = clearTimeoutCalls;
    const retryCountBeforeDuplicate = ws.retryCount;

    expect(pendingReconnectTimeout).toBeDefined();
    expect(retryCountBeforeDuplicate).toBe(1);

    // Duplicate calls should keep the pending reconnect instead of churning
    // another timeout or advancing retry backoff state.
    ws.scheduleReconnect();

    expect(ws.reconnectTimeout).toBe(pendingReconnectTimeout);
    expect(setTimeoutCalls).toBe(setTimeoutCallsBeforeDuplicate);
    expect(clearTimeoutCalls).toBe(clearTimeoutCallsBeforeDuplicate);
    expect(ws.retryCount).toBe(retryCountBeforeDuplicate);
